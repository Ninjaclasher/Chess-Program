...
Piece Piece::make_piece(const int colour, const int x, const int y, const char type, Board* location) 
    {return Piece(colour, x, y, type, location); }
Piece Piece::make_piece(const int x, const int y, const char type, Board* location) 
    {return Piece(2, x, y, type, location); }
Piece Piece::make_piece(Board* location) 
...
void Piece::setColour (const int colour) 
    {this->colour = colour; }
void Piece::setPos (const int x, const int y) 
    {pos = make_pair(x, y); }
void Piece::setPiece (const char type) 
{
...
void Piece::getMoves(vector<Move> &validOptions)
{
    ...
    switch(tolower(type))
    {
        case 'k':
            ...
            if ((x == 0 || x == 7) && thisBoard->board[x][y].inCheck(type).size() == 0 && thisBoard->board[x][y+1].inCheck(type).size() == 0 && thisBoard->board[x][y+2].inCheck(type).size() == 0 && thisBoard->board[x][y+1].getPiece() == '.' && thisBoard->board[x][y+2].getPiece() == '.' && !thisBoard->moved[x][7] && !thisBoard->moved[x][y])
                validOptions.push_back(Move(x, y, x, y+2, x, 7, x, y+1, true));
            if ((x == 0 || x == 7) && thisBoard->board[x][y].inCheck(type).size() == 0 && thisBoard->board[x][y-1].inCheck(type).size() == 0 && thisBoard->board[x][y-2].inCheck(type).size() == 0 && thisBoard->board[x][y-1].getPiece() == '.' && thisBoard->board[x][y-2].getPiece() == '.' && !thisBoard->moved[x][0] && !thisBoard->moved[x][y])
                validOptions.push_back(Move(x, y, x, y-2, x, 0, x, y-1, true));
            for (auto& i : range)
                if (inBound(x+QBRKXY[i].first, y+QBRKXY[i].second) && (thisBoard->board[x+QBRKXY[i].first][y+QBRKXY[i].second].getColour() != getColour() || thisBoard->board[x+QBRKXY[i].first][y+QBRKXY[i].second].getPiece() == '.') && thisBoard->board[x+QBRKXY[i].first][y+QBRKXY[i].second].inCheck(type).size() == 0)
